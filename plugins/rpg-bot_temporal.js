...
try {
let fkontak = { "key": { "participants":"0@s.whatsapp.net", "remoteJid": "status@broadcast", "fromMe": false, "id": "ПРИВЕТ" }, "message": { "contactMessage": { "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD` }}, "participant": "0@s.whatsapp.net" }
...
if (user.joincount === 0 ) throw `${ag}*У ВАС закончилась аренда 🪙*\n\n*СНИМАЙТЕ БОТА, ЧТОБЫ ВЫ МОГЛИ ПРИГЛАСИТЬ ХУЛИГАНА В СВОЮ ГРУППУ С ПОМОЩЬЮ КОМАНДЫ *${usedPrefix}buy joincount 3 o ${usedPrefix}buy Para ver la Tienda*`
 if (!args[1]) throw `${mg}*ИСПОЛЬЗУЙТЕ КОМАНДУ, КАК В ЭТОМ ПРИМЕРЕ*\n*${usedPrefix + command} ссылка и Номер токена(ов)*\n\n*EJEMPLO*\n*${usedPrefix + command} ${nn} 3*\n\n*3 TOKEN 🪙 = 30 MINUTOS*`
if (!linkRegex.test(args[0])) throw `${fg}𝙀𝙉𝙇𝘼𝘾𝙀 𝙉𝙊 𝙑𝘼𝙇𝙄𝘿𝙊.` //Aquí 
...
let data = (await conn.onWhatsApp(jid))[0] || {}
  if (data.exists) 
    conn.reply(m.chat, `@${m.sender.split`@`[0]} adicional ${conn.user.name} a ${await conn.getName(res)}\njid: ${res}, бот выключится вовремя: ${msToDate(global.db.data.chats[res].expired - now)}`, data.jid, m)
                
} conn.sendButton(m.chat, '', wm, null, [['Creadora', `/creadora`], ['𝙑𝙤𝙡𝙫𝙚𝙧 𝙖𝙡 𝙈𝙚𝙣𝙪́ ☘️', '/меню']], m, res) })}
} catch (e) {
await conn.reply(m.chat, `${fg}\`\`\`ВОЗМОЖНЫЕ ПРИЧИНЫ
- Ссылка неверна или просрочена.

- Я не могу присоединиться, если меня ранее исключили из Группы.

- Группа переполнена, поэтому я не могу присоединиться.
\`\`\``, m)
console.log(e)}
}
handler.help = ['temporal', 'tiempo']
handler.command = ['bottemporal', 'botemporal', 'добавить', 'botadd']
